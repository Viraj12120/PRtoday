...
PR Today is a pure Python Terminal CLI application built for performance, aesthetics, and deterministic analysis.

## Core Components

### 1. CLI Layer (`pr_today.cli`)
- Built using **Typer**.
- Handles argument parsing, routing to subcommands (`analyze`, `history`, `auth`), and orchestration.

### 2. UI Layer (`pr_today.ui`)
- Built using **Rich**.
...
### 6. Database Layer (`pr_today.db`)
- Uses **SQLAlchemy** over a local SQLite database (`~/.pr_today/history.db`).
- Stores historical analysis runs for the `history` command.

## Data Flow (Analyze Command)
1. User invokes `pr-today analyze --repo X --pr Y`.
